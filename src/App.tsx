...
import { runRemoteAiPrompt } from './utils/remoteAiClient';
...
const runnerInstructions: Record<Runner, string> = {
  ...
  'Remote AI': 'Remote AI result was processed by the Mac worker through Google Sheets and loaded back into the app.',
...
export default function App() {
  ...
  const handleRun = async () => {
    ...
    if (storage.selectedRunner === 'Remote AI') {
      if (!storage.ollamaModel.trim()) {
        setStatus('Select an Ollama model before running Remote AI');
        return;
      }
      ...
      setRunStartedAt(Date.now());
      setRunTick(0);
      setStatus(`Queued Remote AI job in Google Sheets for ${storage.ollamaModel}...`);

      try {
        const result = await runRemoteAiPrompt(storage.ollamaModel.trim(), prompt);
        updateStorage({
          lastGeneratedPrompt: prompt,
          lastResult: result,
        });
        setStatus('Remote AI result loaded from Google Sheets');
        setModal(runnerInstructions['Remote AI']);
      } catch (error) {
        setStatus(error instanceof Error ? error.message : 'Remote AI run failed');
      } finally {
        setIsRunning(false);
        setRunStartedAt(null);
      }
  ...
  const estimatedTotalSeconds =
    storage.selectedRunner === 'Ollama Local' || storage.selectedRunner === 'Remote AI' ? 180 : 12;
