...
const runnerInstructions: Record<Runner, string> = {
  ...
  'Remote AI (Google Sheets)': 'Remote AI result generated by Ollama, stored in Google Sheets, and loaded back into the app.',
...
export default function App() {
  ...
  const handleRun = async () => {
    ...
    if (storage.selectedRunner === 'Remote AI (Google Sheets)') {
      if (!storage.ollamaModel.trim()) {
      ...
      setRunStartedAt(Date.now());
      setRunTick(0);
      setStatus(`Running Remote AI with ${storage.ollamaModel} and Google Sheets...`);
      ...
      try {
        ...
        setModal(runnerInstructions['Remote AI (Google Sheets)']);
      } catch (error) {
  ...
  const estimatedTotalSeconds =
    storage.selectedRunner === 'Ollama Local' || storage.selectedRunner === 'Remote AI (Google Sheets)' ? 180 : 12;
